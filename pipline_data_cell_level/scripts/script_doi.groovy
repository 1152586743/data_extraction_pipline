...
def pathOutput = "/Users/yhu10/Desktop/test/output"
...
List<String> paint_class_txt_list = ["Stroma", "Tumor"]
...
def get_colors_by_label(class_label){
...
for (roi in ROI_list){
    ...
    for (pc in paint_class_txt_list){
            ...
            for (p in Polygon_annotation_list){
                ...
                if (class_label.contains(c)){
                    ...
                    if (roi.getROI().contains(px, py)){
                        ...
                        brush_color =  get_colors_by_label(class_label)
                        print(class_label)
                        print(brush_color)
    ...
    g2d.dispose()
    def fileMask = new File(pathOutput + File.separator + case_id + File.separator + name + '-mask.png')
